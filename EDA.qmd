...
Number of participants without diabetes vs with prediabetes

The data is very imbalanced, most participants do not have prediabetes.
...
We can expect to that the proportions for participants without prediabetes will be higher since there is a much larger number of those participants overall.

A pattern that sticks out is that the proportion of participants with prediabetes increase with age until ages 75+, when the proportion decreases.

Another pattern is that participants without diabetes have higher proportions for exercising, having health care coverage, eating fruits and veggies, no heart issues, and no heavy alcohol consumption.

```{r}
cat_vars <- names(df)[sapply(df, is.factor)]
# remove Diabetes_binary from the list so we don't two way table with itself
factor_vars <- setdiff(factor_vars, "Diabetes_binary")
...
Not as much difference between the groups of diabetes status as I expected
...
```{r}
ggplot(df, aes(x = Diabetes_binary, fill = Sex)) +
  geom_bar(position='dodge') +
  labs(x = 'Diabetes Status',
       title = 'Diabetes Status by Sex')
```

Diabetes Status grouped by General Health Rating
