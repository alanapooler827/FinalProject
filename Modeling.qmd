...
rec <- recipe(diabetes_status ~ ., data = train)

# recipe using subset of predictor variables
rec2 <- recipe(diabetes_status ~ HighBP + GenHlth + HighChol +
...
Both models have the same accuracy, but the random forest model has a slightly lower log loss than the classification tree model, so random forest is crowned the winner!
